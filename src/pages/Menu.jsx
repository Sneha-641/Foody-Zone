...
const Menu = ({ addToCart }) => {
  ...
  const handleAddToCart = (dish) => {
    ...
    // Temporarily change the icon to "added"
  ...
  const filteredDishes = dishes.filter(
    (dish) => activeFilter === '.all' || dish.category === activeFilter.replace('.', '')
  );
  ...
  return (
    <div className="mx-auto lg:px-28 py-12 text-center md:px-12 px-4" style={{ backgroundImage: `url(${bg})` }} id="menu">
      ...
      <div>
        ...
        <h1 className="md:text-6xl text-3xl text-dark font-[900] mb-12 leading-tight">
      ...
      <div>
        <ul className="inline-block bg-white rounded-full p-2 relative shadow-lg">
          ...
          <li
            className={`inline-flex items-center justify-center cursor-pointer px-6 py-2 mr-2 rounded-full text-xl transition-colors duration-300 ${
              activeFilter === '.all' ? 'bg-prim' : 'hover:bg-white'
            }`}
            onClick={() => handleClick('.all')}
          >
            <img src={m1} alt="" className="mr-2" />
            All
          </li>
          <li
            className={`inline-flex items-center justify-center cursor-pointer px-6 py-2 mr-2 rounded-full text-xl transition-colors duration-300 ${
              activeFilter === '.breakfast' ? 'bg-prim' : 'hover:bg-white'
            }`}
            onClick={() => handleClick('.breakfast')}
          >
            <img src={m2} alt="" className="mr-2" />
            Breakfast
          </li>
          <li
            className={`inline-flex items-center justify-center cursor-pointer px-6 py-2 mr-2 rounded-full text-xl transition-colors duration-300 ${
              activeFilter === '.lunch' ? 'bg-prim' : 'hover:bg-white'
            }`}
            onClick={() => handleClick('.lunch')}
          >
            <img src={m3} alt="" className="mr-2" />
            Lunch
          </li>
          <li
            className={`inline-flex items-center justify-center cursor-pointer px-6 py-2 mr-2 rounded-full text-xl transition-colors duration-300 ${
              activeFilter === '.dinner' ? 'bg-prim' : 'hover:bg-white'
            }`}
            onClick={() => handleClick('.dinner')}
          >
            <img src={m4} alt="" className="mr-2" />
            Dinner
          </li>
        </ul>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 lg:grid-cols-2 md:grid-cols-2 gap-6">
        {filteredDishes.map((dish) => (
          <div
            key={dish.id}
            className="mt-48 text-center bg-gradient-to-br from-gray-200 to-white rounded-3xl shadow-lg p-6 relative transition-transform transform hover:scale-105"
          >
            ...
            <div className="text-xl mt-32 mb-4">
              {dish.rating}
              <span className="text-prim ml-1">
            ...
            <div className="px-8">
              <ul className="flex justify-between items-center">
                ...
                <li>
                  <button
                    className="py-2 px-4 bg-prim rounded-lg shadow-md transition-colors"
                    onClick={() => handleAddToCart(dish)}
                  >
                    <span className="text-white text-xl">
                      <FontAwesomeIcon icon={addedItems[dish.id] ? faCheck : faPlus} />
                    </span>
                  </button>
                </li>
              </ul>
